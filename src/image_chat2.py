...
def main_chat():
    st.title("💬 AI Chat Assistant with Drag-and-Drop Image Input")
    ...
    # Chat input: Allow text and image drag-and-drop
    col1, col2 = st.columns([4, 1])
    with col1:
        prompt = st.chat_input("Type your message or drag an image here:")

    uploaded_image = st.file_uploader("", type=["png", "jpg", "jpeg"])

    if prompt or uploaded_image:
        # Display the user's input or image
        ...
        if prompt:
            ...
            st.session_state.chat_history.append({"role": "user", "message": "[Image Uploaded]"})

        # Generate the AI's response
        if uploaded_image:
            with st.chat_message("assistant"):
                image_response = generate_image_response("Analyze the uploaded image.", uploaded_image.read())
